...
def play_Jarvis():
    ...
    if "play" in instruction:
        talk("Which video you want to play?")
        with aa.Microphone() as origin:
            print("listening..")
            speech = listener.listen(origin)
        ...
        with aa.Microphone() as origin:
            ...
            talk(info)
            # instruction = instruction.lower()
        # humann = instruction.replace('information', " ")
        ...
        talk("Weather of which city you want to know?")
        with aa.Microphone() as origin:
            print("listening..")
            speech = listener.listen(origin)
            ...
            def temp():
                temperature = round(json_data["main"]["temp"] - 273.15,
                                    1)  # Corrected key to "temp", and converted to Celsius
                return temperature

            def des():
                description = json_data["weather"][0]["description"]
                return description
            print(f"Ok. Temperature in {city} is " + str(temp()) +"degree Celcius " + " and It will remain " + str(des()))
            talk(f"Ok. Temperature in {city} is" + str(temp()) + "degree Celcius" + " and It will remain" + str(des()))
